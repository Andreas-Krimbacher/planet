...
// ==== variables ====
    ...
    orbit : null,
    orbitMoon : null,
    rotWorldMatrixForward : null,
    rotWorldMatrixBackward : null,
    rotWorldMatrixMoonForward : null,
    rotWorldMatrixMoonBackward : null,
    rotAxisMoon : null,
    rotAxis : null,
    segments : 16,
    segmentsOrbit : 50,
...
// create the sphere's material
        ...
        if(planetDataObject.moon){
            ...
            this.orbitMoon = {};
            this.rotWorldMatrixMoonForward = {};
            this.rotWorldMatrixMoonBackward = {};
            this.rotAxisMoon = {};
            var moon,meshMoon;
...
//return rotationMatrix
        var rotWorldMatrixForward = new THREE.Matrix4();
        var rotWorldMatrixBackward = new THREE.Matrix4();
        if(moonTranslateX){
            rotWorldMatrixForward.makeRotationAxis(rotAxis, (2*Math.PI) / options.Periode);
            rotWorldMatrixBackward.makeRotationAxis(rotAxis, - (2*Math.PI) / options.Periode);
            this.rotWorldMatrixMoonForward[options.Name] = rotWorldMatrixForward;
            this.rotWorldMatrixMoonBackward[options.Name] = rotWorldMatrixBackward;
            this.rotAxisMoon[options.Name] = rotAxis;
        }
        else{
            rotWorldMatrixForward.makeRotationAxis(rotAxis, (2*Math.PI) / options.Periode);
            rotWorldMatrixBackward.makeRotationAxis(rotAxis, - (2*Math.PI) / options.Periode);
            this.rotWorldMatrixForward = rotWorldMatrixForward;
            this.rotWorldMatrixBackward = rotWorldMatrixBackward;
    ...
    updateForward : function(){
        var rotWorldMatrix;
        if(this.rotWorldMatrixForward){
            rotWorldMatrix = this.rotWorldMatrixForward.clone();
...
// }
        if(this.rotWorldMatrixMoonForward){
            for(var x in this.rotWorldMatrixMoonForward){
                rotWorldMatrix = this.rotWorldMatrixMoonForward[x].clone();
    ...
    updateBackward : function(){
        var rotWorldMatrix;
        if(this.rotWorldMatrixBackward){
            rotWorldMatrix = this.rotWorldMatrixBackward.clone();
            rotWorldMatrix.multiply(this.objectGroup.matrix); // pre-multiply
            this.objectGroup.matrix = rotWorldMatrix;
            this.objectGroup.rotation.setEulerFromRotationMatrix(this.objectGroup.matrix);
        }
...
// this.planetMesh.rotation.y += this.scalePeriod(this.planetDataObject.Revolution)/100;
// }
        if(this.rotWorldMatrixMoonBackward){
            for(var x in this.rotWorldMatrixMoonBackward){
                rotWorldMatrix = this.rotWorldMatrixMoonBackward[x].clone();
                rotWorldMatrix.multiply(this.objectGroupMoon[x].matrix);
                this.objectGroupMoon[x].matrix = rotWorldMatrix;
                this.objectGroupMoon[x].rotation.setEulerFromRotationMatrix(this.objectGroupMoon[x].matrix);
            }
